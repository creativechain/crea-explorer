...
def save_custom_json(op, block, blockid):
    try:
        ...
        pprint("[CREASCAN] - Processing failure")
        pprint(blockid)
        pprint(op['json'])
...
def update_account(account_name):
    ...
    for key in ['balance', 'savings_balance', 'vesting_balance', 'vesting_shares', 'vesting_withdraw_rate', 'cbd_balance', 'savings_cbd_balance']:
...
if __name__ == '__main__':
    pprint("[CREASCAN] - Starting CrearyDB Sync Service")
    sys.stdout.flush()
    ...
    while True:
        ...
        while (block_number - last_block) > 0:
            last_block += 1
            pprint("[CREASCAN] - Starting Block #" + str(last_block))
            sys.stdout.flush()
            # Get full block
            ...
            # if last_block % 100 == 0:
            pprint("[CREASCAN] - Processed up to Block #" + str(last_block))
            sys.stdout.flush()
